...
`editor/` 폴더의 파일은 **열어도 동작하지 않습니다** — 저장이 Claude 안에서만
되기 때문입니다. 코드 백업용입니다.

실제 편집은 Claude 아티팩트 목록의 **HUR Lab Editor** 에서 하고,
Export → Copy → GitHub에 붙여넣기로 반영합니다. 아래 1번에 자세히 적었습니다.
...
### 방법 A — Claude 편집기에서 (권장)

1. Claude 아티팩트 목록에서 **HUR Lab Editor** 를 엽니다
2. 탭을 고르고 내용을 고칩니다 (저장은 자동)
3. 오른쪽 위 **Export** 를 누르면 `content/<이름>.js` 파일 전체가 나옵니다
4. **Copy** 를 누릅니다
5. GitHub에서 그 파일을 열고 → 연필 아이콘 → 전체 선택 → 붙여넣기 → **Commit**
