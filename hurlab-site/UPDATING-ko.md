...
People 탭에서 사람을 펼치면 **Photo** 칸이 있습니다.

1. **Choose a photo…** 를 눌러 사진을 고릅니다
2. 편집기가 알아서 **정사각형으로 자르고 480×480으로 줄이고 JPEG로 압축**합니다
   (원본이 몇 MB든 보통 40~80 KB로 줄어듭니다). 미리보기가 바로 뜹니다
3. **⤓ 파일이름.jpg** 버튼을 눌러 그 파일을 내려받습니다
4. GitHub의 **`hurlab-site/images/`** 폴더에 그 파일을 올립니다
   (Add file → Upload files). **파일 이름은 절대 바꾸지 마세요**
5. 평소처럼 **Get the code** 로 `content/people.js` 를 커밋합니다

**4번과 5번을 둘 다** 해야 합니다. 글자만 올리면 사진이 깨져 보이고, 사진만
올리면 사이트가 그 사진을 모릅니다. Get the code 화면에도 안 올린 사진이
있으면 주황색으로 알려줍니다.

파일 이름은 영문 이름에서 자동으로 만들어집니다 (`Gimin Park` →
`gimin-park.jpg`). 한글 이름이면 `member-m3.jpg` 같은 이름이 붙는데 그대로
쓰시면 됩니다.

사진을 바꾸실 때는 같은 과정을 반복하시면 됩니다. 같은 이름이면 덮어써집니다.
